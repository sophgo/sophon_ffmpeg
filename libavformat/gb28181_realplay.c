...
const AVOption ff_gb28181_options[] = {
...
static int gb28181_read_header(AVFormatContext *s) {
    ...
    if(ret != 0){
        av_log(s, AV_LOG_ERROR,"sip parm parse failed.\n");
    ...
    for(idx=0;idx<SIP_MSG_TIMEOUT;idx++){
        if (get_net_status(&(rt_info->sip_handle))) {
            break;
        }
        usleep(1000);
    }
    ...
    if(idx >= (SIP_MSG_TIMEOUT -1)){
        close_gbclient(&rt_info->sip_handle);
        return -1;
    }
...
#if 0
        FILE *fp = NULL;
        if(fp == NULL) fp = fopen("probecodec.nal","wb+");
...
static int gb28181_read_packet(AVFormatContext *s, AVPacket *pkt) {
    RegistrationInfo *rt = (RegistrationInfo *)s->priv_data;
    if (rt->packet_buffer != NULL) {
        AVPacketList *pktl;
        av_copy_packet(pkt,&(rt->packet_buffer->pkt));
...
static int gb28181_read_close(AVFormatContext *s) {
    RegistrationInfo *rt;
    if(s == NULL) {
        return 0;
    }
...
static const AVClass gb28181_demuxer_class = {
    .class_name     = "GB28181 demuxer",
    .item_name      = av_default_item_name,
    .option         = ff_gb28181_options,
    .version        = LIBAVUTIL_VERSION_INT,
};

AVInputFormat ff_gb28181_demuxer = {
    .name           = "gb28181",
    .long_name      = NULL_IF_CONFIG_SMALL("GB28181 input"),
    .priv_data_size = sizeof(RegistrationInfo),
    .read_probe     = gb28181_probe,
    .read_header    = gb28181_read_header,
    .read_packet    = gb28181_read_packet,
    .read_close     = gb28181_read_close,
    .flags          = AVFMT_NOFILE,
    .priv_class     = &gb28181_demuxer_class,
};
