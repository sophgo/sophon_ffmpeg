...
static void put_avc_eos_tag(AVIOContext *pb, unsigned ts, int is_hevc)
{
...
static void flv_write_codec_header(AVFormatContext* s, AVCodecParameters* par, int64_t ts) {
    ...
    if (par->codec_id == AV_CODEC_ID_AAC || par->codec_id == AV_CODEC_ID_H264 ||
        par->codec_id == AV_CODEC_ID_H265 || par->codec_id == AV_CODEC_ID_MPEG4) {
        int64_t pos;
        avio_w8(pb,
        ...
        if (par->codec_id == AV_CODEC_ID_AAC) {
            ...
            if (par->codec_id == AV_CODEC_ID_H265)
    ...
    if (flv->flags & FLV_NO_SEQUENCE_END) {
        ...
        for (i = 0; i < s->nb_streams; i++) {
            ...
            if (par->codec_type == AVMEDIA_TYPE_VIDEO &&
                    (par->codec_id == AV_CODEC_ID_H264 || par->codec_id == AV_CODEC_ID_H265 ||
                     par->codec_id == AV_CODEC_ID_MPEG4))
                put_avc_eos_tag(pb, sc->last_ts, par->codec_id==AV_CODEC_ID_H265);
        }
    }
...
static int flv_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (par->codec_id == AV_CODEC_ID_VP6F || par->codec_id == AV_CODEC_ID_VP6A ||
        ...
        flags_size = 2;
    else if (par->codec_id == AV_CODEC_ID_H264 || par->codec_id == AV_CODEC_ID_H265 ||
             par->codec_id == AV_CODEC_ID_MPEG4)
        flags_size = 5;
    else
        flags_size = 1;
