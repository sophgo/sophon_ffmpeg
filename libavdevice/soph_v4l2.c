...
static int mmap_init(AVFormatContext *ctx)
{
    ...
    s->buf_paddr = av_malloc_array(s->buffers, sizeof(uint64_t));
    if (!s->buf_paddr) {
        av_log(ctx, AV_LOG_ERROR, "Cannot allocate buffer phy_addr pointers\n");
        av_freep(&s->buf_start);
        av_freep(&s->buf_len);
        return AVERROR(ENOMEM);
    }
...
static int set_wdr_on(int fd, int on)
{
	...
	if (fd > 0) {
		...
		if (ioctl(fd, VIDIOC_S_EXT_CTRLS, &val) < 0) {
			av_log(NULL, AV_LOG_ERROR, "set hdr fail !\n");
			return -1;
		} else {
			av_log(NULL, AV_LOG_DEBUG, "set hdr ctrls id:%d success\n", control.id);
		}
	}
...
static void bm_release_buffer(void *opaque, uint8_t *data)
{
    ...
    bm_free_device(s->bm_handle,*buf_info->bm_mem);
    av_free(buf_info->bm_mem);
    av_free(buf_info);
...
static int try_get_buf_paddr(AVFormatContext *ctx, int fd)
{
    ...
    if(fd > 0) {
		...
		if(ioctl(fd, VIDIOC_G_EXT_CTRLS, &val) < 0) {
            av_log(ctx, AV_LOG_ERROR, "get v4l2 buffer phy addr fail !\n");
			return -1;
		} else {
			av_log(NULL, AV_LOG_INFO,"get v4l2 buffer phy addr success\n");
			for (i = 0; i < s->v4l2_buffer_num; i++) {
                s->buf_paddr[i] = phy_addr[i];
                av_log(NULL, AV_LOG_INFO, "### v4l2 buffer[%d]: 0x%lx\n", i, phy_addr[i]);
            }
		}
	}
    free(phy_addr);
...
static int mmap_read_frame(AVFormatContext *ctx, AVPacket *pkt)
{
    ...
    if (atomic_load(&s->buffers_queued) == FFMAX(s->buffers / 8, 1)) {
        ...
        bm_device_mem_t *frame_buffer;
        bm_device_mem_t queue_buffer;
        ...
        frame_buffer = av_malloc(sizeof(bm_device_mem_t));

        if(res = bm_malloc_device_byte_heap_mask(s->bm_handle, frame_buffer, HEAP_MASK_1, buf.bytesused)!= BM_SUCCESS)
        {
            av_log(ctx, AV_LOG_ERROR,"Cannot allocate new framebuffer!\n");
            return res;
        }

        queue_buffer = bm_mem_from_device(s->buf_paddr[buf.index], buf.bytesused);

        if (res = bm_memcpy_d2d(s->bm_handle, *frame_buffer, 0,
            queue_buffer, 0, buf.bytesused)
            != BM_SUCCESS) {
        ...
        buf_info = av_malloc(sizeof(struct buffer_info));
        if (!buf_info) {
             av_log(ctx, AV_LOG_ERROR, "Failed to allocate a buffer info\n");
             enqueue_buffer(s, &buf);
        ...
        buf_descriptor->s     = s;
        buf_descriptor->buf_info = buf_info;
        ...
        buf_info->idx   = buf.index;
        buf_info->paddr = s->buf_paddr[buf.index];
...
static int mmap_start(AVFormatContext *ctx)
{
    ...
    /* get phy addr */
	res = try_get_buf_paddr(ctx, s->fd);
    if (res < 0) {
        return res;
    }
...
const AVInputFormat ff_soph_v4l2_demuxer = {
    .name           = "Sophgo video4linux2,soph_v4l2",
