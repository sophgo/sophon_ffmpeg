...
#if CONFIG_BMCODEC
    // sophon
    { "bmcodec", bmcodec_init, HWACCEL_BMCODEC, AV_PIX_FMT_BMCODEC },
#endif
#endif
    { 0 },
};
...
#if !defined(BM1684) && defined(BM_PCIE_MODE)
    { "pcie_board_id", HAS_ARG | OPT_INT | OPT_EXPERT | OPT_OUTPUT, { &sophon_device_index },
        "select pcie card by pci_board_id (deprecated)", "pcie board id" },
    { "sophon_device", HAS_ARG | OPT_INT | OPT_EXPERT | OPT_OUTPUT, { &sophon_device_index },
        "select a sophon device", "device" },
    { "zero_copy",  HAS_ARG | OPT_INT | OPT_VIDEO | OPT_INPUT, { &zero_copy },
        "don't copy the decoded image back to CPU in pcie mode", "pcie_no_copyback"},
#endif
    { NULL, },
};
